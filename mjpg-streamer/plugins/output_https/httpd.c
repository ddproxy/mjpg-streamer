...
int _read(SSL* ssl_sock, int fd, iobuffer *iobuf, void *buffer, size_t len, int timeout)
{
    ...
    while((copied < len)) {
         ...
         */
        if((iobuf->level = secure_read(fd, &iobuf->buffer, IO_BUFFER, ssl_sock)) <= 0) {
            /* an error occured */
            return -1;
        }
...
int _readline(SSL* ssl_sock, int fd, iobuffer *iobuf, void *buffer, size_t len, int timeout)
{
    ...
    for(i = 0; i < len && c != '\n'; i++) {
        if(_read(ssl_sock, fd, iobuf, &c, 1, timeout) <= 0) {
...
void send_snapshot(SSL* ssl_sock, int fd, int input_number)
{
    ...
    if((frame = malloc(frame_size + 1)) == NULL) {
        ...
        send_error(ssl_sock, fd, 500, "not enough memory");
        return;
    }
    ...
    if(secure_write(fd, buffer, strlen(buffer), ssl_sock) < 0 || \
            secure_write(fd, frame, frame_size, ssl_sock) < 0) {
        free(frame);
        return;
    }
...
void send_stream(SSL* ssl_sock, int fd, int input_number)
{
    ...
    while(!pglobal->stop) {
        ...
        if(secure_write(fd, buffer, strlen(buffer), ssl_sock) < 0) break;

        DBG("sending frame\n");
        if(secure_write(fd, frame, frame_size, ssl_sock) < 0) break;
...
Return Value: -
******************************************************************************/
void send_file(SSL* ssl_sock, int id, int fd, char *parameter)
{
    char buffer[BUFFER_SIZE] = {0};
    ...
    if(lastDot == 0) {
        send_error(ssl_sock, fd, 400, "No file extension found");
        return;
    } else {
    ...
    if(mimetype == NULL) {
        send_error(ssl_sock, fd, 404, "MIME-TYPE not known");
        return;
    }
    ...
    if((lfd = open(buffer, O_RDONLY)) < 0) {
        ...
        send_error(ssl_sock, fd, 404, "Could not open file");
        return;
    }
    DBG("opened file: %s\n", buffer);
    ...
    do {
        if(secure_write(fd, buffer, i, ssl_sock) < 0) {
            close(lfd);
            return;
        }
...
Return Value: -
******************************************************************************/
void command(int id, SSL* ssl_sock, int fd, char *parameter)
{
    char buffer[BUFFER_SIZE] = {0};
    ...
    if(parameter == NULL || strlen(parameter) >= 255 || strlen(parameter) == 0) {
        ...
        send_error(ssl_sock, fd, 400, "Parameter-string of command does not look valid.");
        return;
    }
    ...
    */
    ...
    if((command = strstr(parameter, "id=")) == NULL) {
        ...
        send_error(ssl_sock, fd, 400, "no GET variable \"id=...\" found, it is required to specify which command id to execute");
        return;
    }
    ...
    if((command = strndup(command, len)) == NULL) {
        send_error(ssl_sock, fd, 500, "could not allocate memory");
...
void *client_thread(void *arg)
{
    ...
    do {
        ...
        if((cnt = _readline(ssl_sock, lcfd.fd, &iobuf, buffer, sizeof(buffer) - 1, 5)) == -1) {
            ...
            SSL_free(ssl_sock);
            close(lcfd.fd);
            DBG("error in _readline, socket closed");
            return NULL;
        }
    ...
    if(!(input_number < pglobal->incnt)) {
        ...
        send_error(ssl_sock, lcfd.fd, 404, "Invalid input plugin number");
        req.type = A_UNKNOWN;
    }
    ...
    case A_SNAPSHOT:
        ...
        send_snapshot(ssl_sock, lcfd.fd, input_number);
        break;
    case A_STREAM:
        DBG("Request for stream from input: %d\n", input_number);
        send_stream(ssl_sock, lcfd.fd, input_number);
        break;
    case A_COMMAND:
        if(lcfd.pc->conf.nocommands) {
            send_error(ssl_sock, lcfd.fd, 501, "this server is configured to not accept commands");
            break;
        }
        command(lcfd.pc->id, ssl_sock, lcfd.fd, req.parameter);
        break;
    case A_INPUT_JSON:
        DBG("Request for the Input plugin descriptor JSON file\n");
        send_Input_JSON(ssl_sock, lcfd.fd, input_number);
        break;
    case A_OUTPUT_JSON:
        DBG("Request for the Output plugin descriptor JSON file\n");
        send_Output_JSON(ssl_sock, lcfd.fd, input_number);
        break;
    case A_PROGRAM_JSON:
        DBG("Request for the program descriptor JSON file\n");
        send_Program_JSON(ssl_sock, lcfd.fd);
        break;
    case A_FILE:
        if(lcfd.pc->conf.www_folder == NULL)
            send_error(ssl_sock, lcfd.fd, 501, "no www-folder configured");
        else
            send_file(ssl_sock, lcfd.pc->id, lcfd.fd, req.parameter);
        break;
    default:
        DBG("unknown request\n");
    }
...
void send_Program_JSON(SSL* ssl_sock, int fd)
{
